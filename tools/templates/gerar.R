...
exemplos <- list(
  ...
  list(pkg = "trama.ml", nome = "Classificação com floresta aleatória",
       ...
       flow = tr_flow(reg) |>
         ...
         tr_add("separar", "ml/split", resposta = "Species", from = "dados") |>
         tr_set("separar", seed = 42L) |>
         tr_add("floresta", "ml/forest", resposta = "Species", trees = 200L, from = "separar:treino") |>
         # Prever e avaliar são os blocos da `models` (contrato `models/fit`).
         tr_add("prever", "models/predict", from = c("floresta", "separar:teste")) |>
         tr_add("avaliar", "models/evaluate", resposta = "Species", from = "prever") |>
         tr_add("confusao", "models/confusion", resposta = "Species", from = "prever")),
  list(pkg = "trama.ml", nome = "Árvore de decisão",
       ...
       flow = tr_flow(reg) |>
         ...
         tr_add("arvore", "ml/cart", resposta = "Species", preditores = "Petal.Length, Petal.Width", from = "dados") |>
