...
`sampling/proportion` para estimar depois.
    ...
    trama::tr_node("sampling/size_stratified", fn = tr_sampling_size_stratified, label = "Tamanho estratificado", version = 2L,
      ...
      params = list(
        estrato = trama::tr_param_col("", label = "Estrato", role = "qualquer", example = "regiao"),
        tamanho = trama::tr_param_col("", label = "Tamanho (N_h)", role = "numerica", example = "N"),
        desvio = trama::tr_param_col("", label = "Desvio (S_h)", role = "numerica", example = "desvio_producao"),
        custo = trama::tr_when(trama::tr_param_col("", label = "Custo (alocação ótima)", role = "numerica", example = "custo"), alocacao = "ótima"),
