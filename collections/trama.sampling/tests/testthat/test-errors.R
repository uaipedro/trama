test_that("toda classe tr_sampling_error_* usada no código está em tr_sampling_errors()", {
  ...
  used <- unique(unlist(regmatches(
    txt, gregexpr('(?<=")tr_sampling_error_[a-z_]+(?=")', txt, perl = TRUE))))
  documented <- tr_sampling_errors()$class
