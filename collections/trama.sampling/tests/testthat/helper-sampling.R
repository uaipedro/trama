...
# 1 — `tr_sampling_srs(populacao, ...)` é chamada de função R comum, sem
# registro e sem motor.

ex <- function(nome) tr_sampling_example(nome)

# A `sampling` NÃO carrega sozinha: as portas são `data/table` (da `data`) e
# `view/plot` (da `view`). A ordem aqui é a mesma que o `trama.json` precisa.
sampling_registry <- function() {
