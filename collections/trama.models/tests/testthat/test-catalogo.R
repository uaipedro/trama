...
test_that("todo nó tem help no formato, e todo campo digitável tem exemplo", {
  ...
  nos <- nos_models(reg)
  expect_length(nos, 56L)  # 9.1b: + gls (main). 9.2: + pr_curve (da ml). Fase 7: predict e rls; coesão F4: confusion, roc, evaluate, importance; F6: scott_knott, dunn, effect_size, cohen_d, plot_coefficients, nls, glmer, plot_regression; main (rigor): friedman, polinomial; 9.2: dose_response fundido no polinomial
  for (n in nos) {
