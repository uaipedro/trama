...
test_that("todo nó tem help no formato, e todo campo digitável tem exemplo", {
  ...
  nos <- nos_models(reg)
  expect_length(nos, 52L)  # Fase 7: predict e rls; coesão F4: confusion, roc, evaluate, importance; F6: scott_knott, dunn, effect_size, cohen_d, plot_coefficients, dose_response, nls, plot_regression
  for (n in nos) {
