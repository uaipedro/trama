...
test_that("o desdobramento bate com os contrastes polinomiais à mão", {
  d <- ex("adubo_dbc")
  r <- tr_models_dose_response(adubo(), "dose")
...
test_that("o grau automático é o maior componente significativo, com a falta de ajuste dele", {
  ...
  expect_equal(tr_models_dose_response(adubo(), "dose", grau = "1")$modelo$grau, 1L)
  expect_error(tr_models_dose_response(tr_models_anova_dic(ex("ToothGrowth"), "len", "dose"), "dose", grau = "3"),
...
test_that("recusas: fator que não é dose, fatorial, poucas doses, nada significativo", {
  expect_error(tr_models_dose_response(milho_dbc(), "hibrido"), class = "tr_models_error_not_numeric")
  fa <- tr_models_anova_factorial(ex("ToothGrowth"), "len", "supp, dose")
  expect_error(tr_models_dose_response(fa, "dose"), class = "tr_models_error_not_applicable")
  pg <- ex("PlantGrowth"); pg$dose <- rep(c(0, 1, 2), each = 10)
  set.seed(3); pg$ruido <- stats::rnorm(30)
  expect_error(tr_models_dose_response(tr_models_anova_dic(pg, "ruido", "dose"), "dose"),
               class = "tr_models_error_not_applicable")
  duas <- ex("ToothGrowth"); duas <- duas[duas$dose != 2, ]
  expect_error(tr_models_dose_response(tr_models_anova_dic(duas, "len", "dose"), "dose"),
               class = "tr_models_error_too_few_rows")
})

test_that("a curva cumpre o contrato e os leitores de ANOVA a recusam com classe", {
  m <- tr_models_dose_response(adubo(), "dose")$modelo
...
test_that("no motor: duas saídas, e o card da curva é o gráfico", {
  ...
  fl <- trama::tr_flow(reg) |>
    ...
    trama::tr_add("reg", "models/dose_response", tratamento = "dose", from = "dbc") |>
