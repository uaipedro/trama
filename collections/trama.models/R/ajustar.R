...
tr_models_glmer <- function(dados, formula = "", resposta = "", fixos = "", grupo = "",
  ...
  # de Wald ficam pequenos demais. A razão de Pearson (a `dispersao_pearson`
  # do `models/fit_stats`) acima de 1,5 vira nota do modelo.
  disp <- if (familia == "poisson") {
    sum(stats::residuals(aj, type = "pearson")^2) / stats::df.residual(aj)
  } else NA_real_
  nota_disp <- if (isTRUE(disp > 1.5)) {
    sprintf(paste0("sobredispersão (razão de Pearson %s): p-valores de Wald ficam pequenos demais; ",
                   "considere o efeito por observação"), .tr_models_fmt(disp))
  } else ""
  .tr_models_fit_obj(aj, "glmer", sprintf("GLM misto · %s", familia), f, p$dados, resp,
                     descartadas = p$descartadas,
                     nota = .tr_models_nota(nota_disp, .tr_models_nota_avisos(r$avisos)))
}
