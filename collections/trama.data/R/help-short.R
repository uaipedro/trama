...
.tr_data_help_curto <- function(id) {
  ajuda <- c(
    ...
    "data/separate" = "## Uso principal\n\nSepara uma coluna de texto em várias pelo separador literal. Número de partes diferente do número de nomes é erro.\n\n## Exemplo curto\n\n`Coluna: codigo`; `Novas colunas: local, tratamento, repeticao`; `Separador: _`\n\n## Usos relacionados\n\n`data/unite` junta colunas; `data/convert` converte as partes numéricas.",
    "data/unite" = "## Uso principal\n\nJunta várias colunas numa só, com um separador entre os valores.\n\n## Exemplo curto\n\n`Colunas: dia, mes, ano`; `Nome: data`; `Separador: /`\n\n## Usos relacionados\n\n`data/separate` faz o inverso; `data/convert` transforma o texto em data.",
    "data/recode" = "## Uso principal\n\nTroca valores de uma coluna (`Níveis: a=x; b=y`) ou fatia um número em faixas (`Cortes: 0; 10; 20`, com rótulos opcionais, fechadas à direita ou à esquerda). Valor fora dos cortes e rótulo a mais ou a menos são erro.\n\n## Exemplo curto\n\n`Coluna: idade`; `Cortes: 0; 18; 60; Inf`; `Rótulos: jovem; adulto; idoso`; `Faixa fechada à: esquerda` (18 anos = adulto). À esquerda, a última faixa fecha nos dois lados; níveis de coluna numérica viram texto.\n\n## Usos relacionados\n\n`data/mutate` para regras livres; `data/convert` para trocar o tipo.",
    "data/sample" = "## Uso principal\n\nSorteia N linhas ou uma fração, com ou sem reposição, opcionalmente dentro de cada grupo. A semente do nó torna a amostra reprodutível.\n\n## Exemplo curto\n\n`N: 5`; `Por grupo: tratamento`\n\n## Usos relacionados\n\n`data/slice_head` pega as primeiras linhas; `data/filter` seleciona por condição.",
