...
sobreposição que o empilhamento costuma produzir."),
      ...
      trama::tr_node("data/to_stream", fn = tr_to_stream, label = "Entrar em fluxo",
        ...
        inputs = list(dados = trama::tr_port(T, required = FALSE)),
...
# tabela se chama `dados` em todas as coleções, e a `data` era a única que
# ainda dizia `data`. Fluxos salvos com a porta velha abrem com as arestas já
# religadas na nova — sem isso, cada aresta viraria "porta inexistente".
.tr_data_migracoes <- function() {
  nos <- c("arrange", "clean_names", "convert", "distinct", "drop_na", "filter",
           "from_stream", "get_dupes", "group_summarise", "mutate",
           "pivot_longer", "pivot_wider", "remove_empty", "rename",
           "replace_na", "select", "slice_head", "summary", "to_stream",
           "write_csv", "write_parquet", "write_rds")
  stats::setNames(rep(list(list(data = "dados")), length(nos)), paste0("data/", nos))
}
