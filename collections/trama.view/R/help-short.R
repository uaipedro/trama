...
.tr_view_help_curto <- function(id) {
  ajuda <- c(
    ...
    "view/reference" = "## Uso principal\n\nAcrescenta ao gráfico linhas de referência (limite, meta, y = x) e, se quiser, uma faixa sombreada; aparece em todos os painéis.\n\n## Exemplo curto\n\n`Tipo: horizontal`; `Valor: 10; 20`; `Texto: mín.; máx.`; `Faixa até: 25` (com um valor só)\n\n## Usos relacionados\n\nVírgula é decimal e `;` separa valores. Ligue antes do painel: `view/combine` não aceita camada por cima. `view/fit_line` desenha a reta ajustada.",
    "view/fit_line" = "## Uso principal\n\nAcrescenta a um disperso a reta ou curva ajustada aos próprios pontos, com intervalo de confiança da média e a equação com R².\n\n## Exemplo curto\n\n`Método: linear`; `Intervalo: ligado`; `Confiança (IC): 0,95`; `Equação e R²: ligado`; `Posição da equação: automática`\n\n## Usos relacionados\n\nUm ajuste por painel e, com `Uma por cor`, por grupo de cor; loess não tem equação. `view/points` com Tendência dá a reta sem equação; a regressão com teste fica na coleção de modelos.",
    "view/annotate" = "## Uso principal\n\nEscreve um texto numa coordenada do gráfico, com seta opcional até outro ponto.\n\n## Exemplo curto\n\n`X: 4`; `Y: 30`; `Texto: carros leves`; `Seta até X: 2`; `Seta até Y: 32`\n\n## Usos relacionados\n\nCoordenadas nas unidades dos eixos, vírgula decimal. `view/labels` escreve o nome de todos os pontos.",
    "view/combine" = "## Uso principal\n\nJunta gráficos numa figura com painéis etiquetados (A, B, C), com um tema só para todos.\n\n## Exemplo curto\n\n`Por linha: 2`; `Etiquetas: A, B, C`; `Legenda comum: ligado`\n\n## Usos relacionados\n\nA ordem dos painéis é a ordem em que os gráficos foram ligados; `view/save` grava o painel no tamanho do periódico.",
    "view/save" = "## Uso principal\n\nGrava o gráfico ou o painel em PDF, PNG ou TIFF, na largura em milímetros e na resolução que o periódico pede.\n\n## Exemplo curto\n\n`Arquivo: figuras/figura1.pdf`; `Formato: pdf`; `Largura (mm): 85`; `Altura (mm): 0`; `Texto (pt): 9`\n\n## Usos relacionados\n\nAltura 0 segue a proporção do gráfico; o texto sai no tamanho impresso de Texto (pt), 8–10 pt como pedem os periódicos. `view/combine` monta o painel antes; `data/write_csv` grava a tabela de origem.",
...
.tr_view_aplicar_ajuda_curta <- function(colecao) {
  guias <- c(
    ...
    "view/reference" = "https://uaipedro.github.io/trama/colecoes/visualizacao/referencia/",
    "view/fit_line" = "https://uaipedro.github.io/trama/colecoes/visualizacao/reta-ajustada/",
    "view/annotate" = "https://uaipedro.github.io/trama/colecoes/visualizacao/anotacao/",
