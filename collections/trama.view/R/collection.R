...
trama_collection <- function() {
  ...
  .tr_view_aplicar_ajuda_curta(trama::tr_collection(
    ...
    categories = list(
      ...
      # Camadas antes da figura: anota-se o gráfico, e só depois ele entra no
      # painel (que recusa camada por cima). Papel de inspeção, como os
      # gráficos que elas completam.
      trama::tr_category("camadas",     "Camadas", role = "inspecao"),
    ...
    nodes = c(list(
      trama::tr_node("view/points", fn = tr_points, label = "Disperso",
        ...
        params = .tr_view_props(
          x   = trama::tr_param_col("", label = "Eixo X", role = "numerica", example = "valor"),
          y   = trama::tr_param_col("", label = "Eixo Y", role = "numerica", example = "qtd"),
...
ser categoria.", .TR_VIEW_AJUDA_APARENCIA)),
      ...
      trama::tr_node("view/line", fn = tr_line, label = "Linha",
        ...
        params = .tr_view_props(
          ...
          cor = trama::tr_param_col("", label = "Uma linha por", role = "categorica", example = "regiao", suggest = FALSE),
...
código numérico que devia ser categoria.", .TR_VIEW_AJUDA_APARENCIA))),
    ...
    list(
      trama::tr_node("view/bars", fn = tr_bars, label = "Barras",
        ...
        inputs = list(dados = T), outputs = list(out = G),
        params = .tr_view_props(
          x   = trama::tr_param_col("", label = "Categoria", role = "categorica", example = "regiao"),
          y   = trama::tr_param_col("", label = "Altura", role = "numerica", example = "receita_total"),
          cor = trama::tr_param_col("", label = "Subgrupo", role = "categorica", example = "produto", suggest = FALSE),
...
`data/convert` quando a altura veio como texto.", .TR_VIEW_AJUDA_APARENCIA))),
    ...
    .tr_view_nos_camadas(P, G), .tr_view_nos_figura(P, G), list(.tr_view_no_salvar(P, G))),
