  ...
  list(
    trama::tr_node("experiments/effect", version = 2L, fn = tr_experiments_effect, label = "Efeito",
      ...
      params = list(
        ...
        fator = trama::tr_when(P("text", "", label = "Fator(es)", example = "bloco:parcela"),
          tipo = c("fixo", "aleatorio", "interacao", "quantitativo", "covariavel")),
        valor = trama::tr_when(N(0, label = "Valor (intercepto)"), tipo = "intercepto"),
        efeitos = trama::tr_when(P("text", "", label = "Efeitos por nível ou célula", example = "baixa = 0, alta = 2"),
          tipo = c("fixo", "interacao"), conjunto = "por nível"),
        conjunto = trama::tr_when(E("por nível", .TR_EXP_EF_CONJUNTOS, label = "Declarar por"), tipo = c("fixo", "interacao")),
        magnitudes = trama::tr_when(P("text", "", label = "Magnitudes dos contrastes", example = "linear = 4, quadratico = 1, cubico = 0"),
          tipo = c("fixo", "interacao"), conjunto = c("polinomiais", "helmert", "controle", "digitados")),
        doses = trama::tr_when(P("text", "", label = "Doses (valores dos níveis)", example = "0, 50, 100, 150"),
          tipo = c("fixo", "interacao", "quantitativo")),
        controle = trama::tr_when(P("text", "", label = "Controle", example = "testemunha"), tipo = "fixo", conjunto = "controle"),
        contrastes = trama::tr_when(P("expr", "", label = "Contrastes (digitados)", example = "A vs B: A - B"),
          tipo = "fixo", conjunto = "digitados"),
        sd = trama::tr_when(N(1, min = 0, label = "Desvio-padrão (aleatório; covariável gerada)"), tipo = c("aleatorio", "covariavel")),
        coeficientes = trama::tr_when(P("text", "", label = "Coeficientes (quantitativo)", example = "0.5, -0.02"), tipo = "quantitativo"),
        inclinacao = trama::tr_when(N(0, label = "Inclinação (covariável)"), tipo = "covariavel"),
        media = trama::tr_when(N(0, label = "Média (covariável)"), tipo = "covariavel"),
    ...
    trama::tr_node("experiments/error", fn = tr_experiments_error, label = "Erro",
      ...
      params = list(
        ...
        sd = trama::tr_when(N(1, min = 0, label = "Desvio-padrão (normal)"), distribuicao = "normal"),
        sd_por = P("text", "", label = "sd por nível de", example = "irrigacao"),
        sds = P("text", "", label = "sd de cada nível", example = "baixa = 1, alta = 3"),
        ensaios = trama::tr_when(I(10L, min = 1L, max = 1000000L, label = "Ensaios (binomial)"), distribuicao = "binomial"),
        forma = trama::tr_when(N(2, min = 0, label = "Forma (gama)"), distribuicao = "gama"),
        correlacao = E("independente", .TR_EXP_ER_COR, label = "Correlação no indivíduo"),
        rho = trama::tr_when(N(0.5, min = -1, max = 1, step = 0.05, label = "ρ"), correlacao = c("simetria_composta", "ar1")),
