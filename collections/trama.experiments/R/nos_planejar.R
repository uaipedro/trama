  ...
  list(
    trama::tr_node("experiments/design", version = 4L, fn = tr_experiments_design, label = "Delineamento",
      ...
      params = list(
        ...
        delineamento_base = trama::tr_when(E("dbc", c("dbc", "dic"), label = "Base (fatorial, subdividida, grupos)"),
          estrutura = c("fatorial", "parcela_subdividida", "grupos")),
        confundir = trama::tr_when(P("text", "", label = "Confundir com blocos", example = "ABC"), estrutura = "confundimento"),
        geradores = trama::tr_when(P("text", "", label = "Geradores do fracionado", example = "D = ABC"),
          estrutura = c("fracionado", "composto_central")),
        distancia_axial = trama::tr_when(P("text", "rotacional", label = "Distância axial α (composto central)",
                            example = "rotacional, face ou 1.5"), estrutura = "composto_central"),
        pontos_centrais = trama::tr_when(I(4L, min = 0L, max = 50L, label = "Pontos centrais"), estrutura = "composto_central"),
        tamanho_bloco = trama::tr_when(I(3L, min = 2L, max = 100L, label = "Parcelas por bloco (BIB)"), estrutura = "bib"),
        tempos = trama::tr_when(P("text", "", label = "Tempos (medidas repetidas)", example = "0, 30, 60, 90"),
                                estrutura = "medidas_repetidas"),
        locais = trama::tr_when(I(3L, min = 1L, max = 100L, label = "Locais (grupos)"), estrutura = "grupos"),
        colunas_grade = trama::tr_when(I(0L, min = 0L, max = 1000L, label = "Colunas da grade (0 = automática)"),
          estrutura = c("dic", "fatorial", "fracionado", "composto_central", "parcela_subdividida", "grupos")),
