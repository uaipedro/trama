...
.tr_ml_docs_modelos <- function() {
  ...
  imp_arvore <- function(medida) P(sprintf(
    "A **importância** (no `models/importance`) é %s, medida no treino: descreve o ajuste, não o efeito. Tende a favorecer preditores contínuos ou com muitos valores distintos, e preditores correlacionados repartem entre si a importância.", medida),
    verificar = "data/summary",
    ...
    "ml/linear" = list(
      pressupostos = c(base, list(num,
        ...
        P("Na classificação, **duas classes**, e a classe prevista é a segunda quando sua probabilidade é ≥ `corte` (padrão 0,5). Com classes desequilibradas o corte 0,5 favorece a maioritária.",
          ...
          se_falhar = "Leia a `balanced_accuracy` no `models/evaluate` e a curva no `models/roc`, que não dependem do corte. Se mudar o `corte`, escolha-o com dados de treino (validação), nunca olhando o teste."))),
    ...
    "ml/cart" = list(
      pressupostos = c(base, list(num,
        P("A árvore cresce até `max_depth`, `min_n` e `cp` e é **podada por custo-complexidade**: entre as subárvores aninhadas, a validação cruzada do `rpart` com min(10, n) folds (dentro do treino, com a semente do bloco; com n ≤ 10 vira deixa-um-fora, e com poucas dezenas de linhas o erro de validação é muito variável e a poda, instável) escolhe a menor cujo erro não passa do mínimo mais um erro-padrão (regra 1-EP, `poda = \"1ep\"`). `poda = \"minimo\"` fica com a de menor erro, maior e mais variável; `\"nenhuma\"` desliga a poda.",
          verificar = c("ml/tune", "models/evaluate"),
