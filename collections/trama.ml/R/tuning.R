...
# Sem `preditores`, são os numéricos exceto resposta, ordem e grupo.
.tr_ml_cols_sem_aux <- function(dados, resposta, preditores, ordem, grupo) {
  if (length(.tr_ml_cols(preditores))) return(preditores)
  num <- names(dados)[vapply(dados, is.numeric, TRUE)]
  paste(setdiff(num, c(resposta, trimws(ordem), trimws(grupo))), collapse = ", ")
}
...
#' @param grupo Coluna de grupo para `estrategia = "grupo"`. Com `preditores` vazio,
...
tr_ml_tune <- function(dados, resposta = "", preditores = "", modelo = "cart", tarefa = "auto",
  ...
  preditores <- .tr_ml_cols_sem_aux(dados, resposta, preditores, ordem, grupo)
...
#' tr_ml_nested_cv(d, resposta = "Species", tentativas = 3, folds_externos = 3, folds = 3)
#' @export
tr_ml_nested_cv <- function(dados, resposta = "", preditores = "", modelo = "cart", tarefa = "auto",
  ...
  partes <- .tr_ml_with_seed(seed, {
    y <- if (tarefa == "classificacao") factor(dados[[d$resposta]]) else dados[[d$resposta]]
    .tr_ml_folds(dados, estrategia, folds_externos, y, ordem, grupo)
  })
  y_ext <- dados[[d$resposta]]
  ...
  linhas <- .tr_ml_sem_impressao(lapply(seq_along(partes), function(i) {
    ...
    z <- tr_ml_tune(treino, resposta = resposta, preditores = preditores, modelo = modelo, tarefa = tarefa,
    ...
    # A previsão crua do contrato (antes, a extinta `ml/predict`).
    pred <- .tr_ml_prever(z$modelo, .tr_ml_novos_dados(z$modelo, validacao))$previsto
    tibble::tibble(fold = as.character(i), n_treino = nrow(treino), n_validacao = nrow(validacao),
                   ...
                   externa = .tr_ml_tune_metric(validacao[[d$resposta]], pred, tarefa, z$metrica))
  }))
