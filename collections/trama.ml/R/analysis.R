...
#' @param dados Tabela com a classe observada e a probabilidade prevista
#'   (a saída de `models/predict`, com as colunas `prob_<classe>`).
#' @param resposta Nome da coluna observada.
...
#'   coluna (`prob_<classe>`, como sai de `models/predict`); se o nome não
#'   indicar uma classe observada, o bloco recusa
#'   (`tr_ml_error_positive_required`) em vez de adivinhar.
#' @param aspecto,tema,titulo,rotulo_x,rotulo_y,legenda Acabamento do gráfico,
#'   como nos demais gráficos da coleção.
...
#' @export
tr_ml_pr_curve <- function(dados, resposta = "", probabilidade = "", positiva = "",
  ...
  if (!nzchar(positiva)) positiva <- .tr_ml_roc_positiva(dados[[resposta]], probabilidade)
...
# Classe positiva padrão: a que a coluna `prob_<classe>` nomeia (o nome que a
# `models/predict` dá; o `.prob_<classe>` da antiga `ml/predict` ainda vale,
# para tabelas geradas antes da migração). Sem esse
...
.tr_ml_roc_positiva <- function(y, probabilidade) {
  ...
  if (grepl("^\\.?prob_", probabilidade) && da_coluna %in% niveis) return(da_coluna)
