...
.tr_ml_migracoes <- function() {
  ...
  ajustes <- c("linear", "cart", "figs", "forest", "svm", "xgboost", "tune")
  so_resposta <- c("split", "evaluate", "confusion", "residuals", "roc")
  c(stats::setNames(rep(list(ajuste), length(ajustes)), paste0("ml/", ajustes)),
    stats::setNames(rep(list(resposta), length(so_resposta)), paste0("ml/", so_resposta)))
}
...
.tr_ml_workflow_nodes <- function() {
  ...
  list(
    trama::tr_node("ml/example", tr_ml_example, label = "Dados para aprender",
      ...
      category = "ml_dados", outputs = list(out = "data/table"),
    ...
    trama::tr_node("ml/split", tr_ml_split, label = "Separar treino / teste",
      ...
      category = "ml_dados", inputs = list(dados = "data/table"),
      ...
      help = .tr_ml_help("Reserve o teste antes de ajustar modelos. A sa\u{ED}da treino alimenta os ajustes; teste alimenta apenas Prever. A divis\u{E3}o aleat\u{F3}ria sup\u{F5}e observa\u{E7}\u{F5}es independentes: para tempo, indiv\u{ED}duos repetidos ou grupos, separe por tempo/grupo fora deste bloco.",
        "`resposta`: coluna a prever; quando categ\u{F3}rica, permite estratificar. `proporcao`: fra\u{E7}\u{E3}o aproximada do treino; arredondamento por classe pode mudar a fra\u{E7}\u{E3}o final. `estratificar`: mant\u{E9}m cada classe nos dois conjuntos; classes unit\u{E1}rias s\u{E3}o recusadas. Para classes num\u{E9}ricas, converta a fator antes. `seed`: reproduz a divis\u{E3}o sem modificar o gerador da sess\u{E3}o.",
    ...
    trama::tr_node("ml/predict", role = "leitura", tr_ml_predict, label = "Prever",
      ...
      help = .tr_ml_help("Use dados de teste ou dados novos com os mesmos preditores do treino. A resposta n\u{E3}o \u{E9} necess\u{E1}ria para prever. Colunas de sa\u{ED}da existentes s\u{E3}o recusadas para n\u{E3}o sobrescrever dados.",
        ...
        "d <- trama.ml::tr_ml_split(trama.ml::tr_ml_example(), resposta = 'Species')\nm <- trama.ml::tr_ml_cart(d$treino, resposta = 'Species')\ntrama.ml::tr_ml_predict(m, d$teste)", "`ml/evaluate`, `ml/confusion`.")),
    trama::tr_node("ml/evaluate", tr_ml_evaluate, label = "Avaliar previs\u{F5}es",
      description = "Calcula erros de regress\u{E3}o ou m\u{E9}tricas de classifica\u{E7}\u{E3}o nas linhas fornecidas.",
      category = "ml_avaliar", inputs = list(dados = "data/table"), outputs = list(out = "data/table"),
      params = list(resposta = resposta, predito = predito, tarefa = .tr_ml_task_param()),
      help = .tr_ml_help("Mede a compara\u{E7}\u{E3}o entre resposta observada e previs\u{E3}o. O bloco n\u{E3}o sabe se voc\u{EA} conectou treino ou teste: use a sa\u{ED}da teste da divis\u{E3}o. Reutilizar o teste para escolher hiperpar\u{E2}metros torna sua estimativa otimista.",
        "`resposta`: resposta observada. `predito`: coluna prevista. `tarefa`: auto, regressao ou classificacao.",
        ...
        "d <- data.frame(y = c(1, 2, 3), .pred = c(1, 2, 4))\ntrama.ml::tr_ml_evaluate(d, resposta = 'y')", "`ml/predict`, `ml/confusion`.")),
    trama::tr_node("ml/confusion", tr_ml_confusion, label = "Matriz de confus\u{E3}o",
      description = "Conta acertos e confus\u{F5}es entre classes observadas e previstas.",
      category = "ml_avaliar", inputs = list(dados = "data/table"), outputs = list(out = "data/table"),
      params = list(resposta = resposta, predito = predito),
      help = .tr_ml_help("Mostra todas as combina\u{E7}\u{F5}es de classes, inclusive contagens zero. Leia em conjunto com o tamanho de cada classe; a acur\u{E1}cia sozinha pode esconder a classe minorit\u{E1}ria.",
        "`resposta`: classe observada. `predito`: classe prevista.", "Tabela observado, previsto e n.",
        "d <- data.frame(y = c('a', 'a', 'b'), .pred = c('a', 'b', 'b'))\ntrama.ml::tr_ml_confusion(d, resposta = 'y')", "`ml/evaluate`, `ml/predict`.")),
    trama::tr_node("ml/rules", tr_ml_rules, label = "Ler regras das \u{E1}rvores",
      ...
      category = "ml_inspecionar", inputs = list(modelo = "ml/fit"), outputs = list(out = "data/table"),
