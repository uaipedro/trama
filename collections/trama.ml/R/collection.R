...
trama_collection <- function() {
  trama::tr_collection("ml", version = "0.3.0", label = "Machine learning",
...
.tr_ml_workflow_nodes <- function() {
  ...
  list(
    ...
    trama::tr_node("ml/split", tr_ml_split, version = 2L, label = "Separar treino / teste",
      ...
      help = .tr_ml_help("Reserve o teste antes de ajustar modelos. A sa\u{ED}da treino alimenta os ajustes; teste alimenta apenas Prever. A divis\u{E3}o aleat\u{F3}ria sup\u{F5}e observa\u{E7}\u{F5}es independentes; com tempo use `estrategia = temporal` (teste inteiro depois do treino) e com indiv\u{ED}duos, lotes ou \u{E1}reas repetidos use `estrategia = grupo` (nenhum grupo dos dois lados).",
        "`alvo`: resposta; quando categ\u{F3}rica, permite estratificar. `proporcao`: fra\u{E7}\u{E3}o aproximada do treino (dos grupos, na estrat\u{E9}gia grupo); arredondamento por classe pode mudar a fra\u{E7}\u{E3}o final. `estratificar`: mant\u{E9}m cada classe nos dois conjuntos; classes unit\u{E1}rias s\u{E3}o recusadas; ignorado nas estrat\u{E9}gias temporal e grupo. Para classes num\u{E9}ricas, converta a fator antes. `estrategia`: aleatoria, temporal (treino = linhas at\u{E9} o instante da linha floor(n \u{B7} propor\u{E7}\u{E3}o) na ordem do tempo, empates juntos) ou grupo (sorteia grupos inteiros). `ordem`: coluna de tempo (n\u{FA}mero, data ou data-hora). `grupo`: coluna de grupo. `seed`: reproduz a divis\u{E3}o sem modificar o gerador da sess\u{E3}o.",
        "Duas tabelas, treino e teste, sem duplicar nem perder linhas, marcadas com a proveni\u{EA}ncia (atributo `tr_ml_origem`): ajustar um modelo no teste \u{E9} recusado (`tr_ml_error_test_leak`) e avaliar o treino exige `permitir_treino`. A marca sobrevive ao cache e a filtros; juntar treino e teste a perde.",
    ...
    trama::tr_node("ml/predict", role = "leitura", tr_ml_predict, version = 2L, label = "Prever",
      ...
      help = .tr_ml_help("Use dados de teste ou dados novos com os mesmos preditores do treino. A resposta n\u{E3}o \u{E9} necess\u{E1}ria para prever. Colunas de sa\u{ED}da existentes s\u{E3}o recusadas para n\u{E3}o sobrescrever dados.",
        ...
        "Tabela original com `.pred` e, quando dispon\u{ED}veis, probabilidades `.prob_<classe>`; a marca treino/teste do `ml/split` passa adiante. Prever o teste de outra divis\u{E3}o com um modelo do treino de uma divis\u{E3}o marcada \u{E9} recusado (`tr_ml_error_split_mismatch`).",
    ...
    trama::tr_node("ml/evaluate", tr_ml_evaluate, version = 3L,
      ...
      params = list(alvo = alvo, predito = predito, tarefa = .tr_ml_task_param(), permitir_treino = trama::tr_param_bool(FALSE, label = "Permitir avaliar o treino")),
      help = .tr_ml_help("Mede a compara\u{E7}\u{E3}o entre resposta observada e previs\u{E3}o.",
        "`alvo`: resposta observada. `predito`: coluna prevista. `tarefa`: auto, regressao ou classificacao. `permitir_treino`: desligado, recusa previs\u{F5}es das linhas de treino do `ml/split` (erro `tr_ml_error_train_eval`); ligado, avalia assim mesmo e acrescenta a coluna `nota` sobre o otimismo. Tabelas sem a marca do `ml/split` s\u{E3}o avaliadas como chegam.",
        "Tabela de m\u{E9}tricas (colunas metrica, classe, valor, n): RMSE, MAE e R\u{B2} para regress\u{E3}o; na classifica\u{E7}\u{E3}o, acur\u{E1}cia, acur\u{E1}cia balanceada (m\u{E9}dia das revoca\u{E7}\u{F5}es), macro F1, kappa de Cohen, precis\u{E3}o e revoca\u{E7}\u{E3}o macro, precis\u{E3}o, revoca\u{E7}\u{E3}o e F1 ponderados pelo suporte e, por classe, precis\u{E3}o, revoca\u{E7}\u{E3}o e F1 (n = suporte da classe). R\u{B2} \u{E9} indefinido para resposta constante; o kappa, quando s\u{F3} h\u{E1} um r\u{F3}tulo. M\u{E9}dias macro usam as classes observadas; precis\u{E3}o de classe nunca prevista vale 0.",
        "d <- data.frame(y = c(1, 2, 3), .pred = c(1, 2, 4))\ntrama.ml::tr_ml_evaluate(d, alvo = 'y')", "`ml/predict`, `ml/confusion`.")),
    trama::tr_node("ml/confusion", tr_ml_confusion, version = 2L,
      ...
      params = list(alvo = alvo, predito = predito, permitir_treino = trama::tr_param_bool(FALSE, label = "Permitir avaliar o treino")),
      help = .tr_ml_help("Mostra todas as combina\u{E7}\u{F5}es de classes, inclusive contagens zero.",
        "`alvo`: classe observada. `predito`: classe prevista. `permitir_treino`: desligado, recusa previs\u{F5}es das linhas de treino do `ml/split` (erro `tr_ml_error_train_eval`); ligado, avalia assim mesmo e acrescenta a coluna `nota` sobre o otimismo. Tabelas sem a marca do `ml/split` s\u{E3}o avaliadas como chegam.", "Tabela observado, previsto e n.",
