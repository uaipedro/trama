  ...
  list(
    ...
    trama::tr_node("ml/split", tr_ml_split, version = 3L, label = "Separar treino / teste",
      ...
      params = list(resposta = resposta, proporcao = trama::tr_param_num(0.75, min = 0.01, max = 0.99, label = "Fra\u{E7}\u{E3}o para treino"),
        estratificar = trama::tr_when(trama::tr_param_bool(TRUE, label = "Estratificar classes"), estrategia = "aleatoria"),
        estrategia = .tr_ml_estrategia_param(),
        ordem = trama::tr_when(.tr_ml_ordem_param(), estrategia = "temporal"),
        grupo = trama::tr_when(.tr_ml_grupo_param(), estrategia = "grupo"),
