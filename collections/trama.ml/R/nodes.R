...
#' @return Um modelo `tr_ml_fit`.
#' @export
tr_ml_linear <- function(dados, resposta = "", preditores = "", tarefa = "auto", corte = 0.5, seed = 42L) {
  tr_ml_fit(dados, resposta, preditores, "linear", tarefa, seed = seed, corte = corte)
}
...
.tr_ml_ordem_param <- function() trama::tr_param_col("", label = "Coluna de tempo", role = "qualquer", example = "data", suggest = FALSE)
.tr_ml_grupo_param <- function() trama::tr_param_col("", label = "Coluna de grupo", role = "categorica", example = "lote", suggest = FALSE)
...
.tr_ml_model_nodes <- function() {
  ...
  configs <- list(
    linear = list(fn = tr_ml_linear, icon = "chart-spline", label = "Linear / log\u{ED}stica", category = "ml_simples",
      ...
      params = list(corte = trama::tr_when(trama::tr_param_num(0.5, min = 0.000001, max = 0.999999, label = "Corte de probabilidade"),
                                         tarefa = c("auto", "classificacao"))),
    ...
    svm = list(fn = tr_ml_svm, icon = "move-diagonal", label = "SVM \u{B7} vetores de suporte", category = "ml_margem",
      ...
      params = list(kernel = trama::tr_param_enum("radial", c("linear", "radial", "polynomial", "sigmoid"), label = "Kernel"),
        cost = trama::tr_param_num(1, min = 0.000001, label = "Custo"),
        gamma = trama::tr_when(trama::tr_param_num(0.1, min = 0.000001, label = "Gamma"),
                               kernel = c("radial", "polynomial", "sigmoid"))),
      extra = "`kernel`: geometria da fronteira. `cost`: penalidade dos erros. `gamma`: escala do kernel; n\u{E3}o \u{E9} usado no kernel linear."),
  ...
  list(
    ...
    trama::tr_node("ml/nested_cv", role = "avaliacao", tr_ml_nested_cv, version = 3L,
      ...
      help = .tr_ml_help("Cada fold externo roda um `ml/tune` completo s\u{F3} no seu treino e mede o vencedor na sua valida\u{E7}\u{E3}o, que a busca nunca viu. A m\u{E9}dia `externa` estima o desempenho do procedimento; a `interna` mostra o otimismo da sele\u{E7}\u{E3}o.",
        ...
        "d <- trama.ml::tr_ml_example('iris_binaria')\ntrama.ml::tr_ml_nested_cv(d, resposta = 'Species', tentativas = 3, folds_externos = 3, folds = 3)",
        "`ml/tune`, `ml/split`, `models/evaluate`.")),
