  ...
  list(
    ...
    trama::tr_node("ml/roc", role = "avaliacao", tr_ml_roc, version = 5L,
      ...
      help = .tr_ml_help("Ordena as linhas pela probabilidade da classe positiva e exibe a curva ROC com sua AUC, o intervalo de confian\u{E7}a de DeLong para a AUC e o corte de Youden (maior sensibilidade + especificidade \u{2212} 1), marcado em vermelho. Use somente classifica\u{E7}\u{E3}o bin\u{E1}ria.",
        "`alvo`: classe observada. `probabilidade`: coluna `.prob_<classe>` criada por Prever. `positiva`: classe correspondente; vazio deduz a classe do nome da coluna (`.prob_<classe>`); se o nome não indicar uma classe observada, o bloco pede `positiva` em vez de adivinhar. `confianca`: n\u{ED}vel do intervalo da AUC. `permitir_treino`: desligado, recusa previs\u{F5}es das linhas de treino do `ml/split` (erro `tr_ml_error_train_eval`); ligado, avalia assim mesmo e acrescenta a nota na legenda e na coluna `nota` sobre o otimismo. Tabelas sem a marca do `ml/split` s\u{E3}o avaliadas como chegam.",
    ...
    trama::tr_node("ml/pr_curve", role = "avaliacao", tr_ml_pr_curve, version = 3L,
      ...
      help = .tr_ml_help("Ordena as linhas pela probabilidade da classe positiva e mostra, em cada corte, a precis\u{E3}o contra a revoca\u{E7}\u{E3}o, com a precis\u{E3}o m\u{E9}dia (AP) e a preval\u{EA}ncia como linha do acaso. Prefira \u{E0} ROC quando a classe de interesse \u{E9} rara.",
        "`alvo`: classe observada. `probabilidade`: coluna `.prob_<classe>` criada por Prever. `positiva`: classe correspondente; vazio deduz a classe do nome da coluna (`.prob_<classe>`); se o nome não indicar uma classe observada, o bloco pede `positiva` em vez de adivinhar. `permitir_treino`: desligado, recusa previs\u{F5}es das linhas de treino do `ml/split` (erro `tr_ml_error_train_eval`); ligado, avalia assim mesmo e acrescenta a nota na legenda e na coluna `nota` sobre o otimismo. Tabelas sem a marca do `ml/split` s\u{E3}o avaliadas como chegam.",
