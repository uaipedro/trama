...
# Desde a Fase 4 da coesão, prever e avaliar são os blocos da `trama.models`
# (`models/predict`, `models/evaluate`, `models/confusion`, `models/roc`,
# `models/pr_curve`), que leem a marca pelo contrato. Os testes da main (da77947,
# 63ec621) ficam como estavam, com estes atalhos no lugar dos blocos da ml.
tr_ml_predict <- function(modelo, dados) trama.models::tr_models_predict(modelo, dados)
tr_ml_evaluate <- function(dados, resposta, permitir_treino = FALSE) {
  trama.models::tr_models_evaluate(dados = dados, resposta = resposta, permitir_treino = permitir_treino)
}
tr_ml_confusion <- function(dados, resposta, permitir_treino = FALSE) {
  trama.models::tr_models_confusion(dados = dados, resposta = resposta, permitir_treino = permitir_treino)
}
tr_ml_roc <- function(dados, resposta, probabilidade, permitir_treino = FALSE) {
  trama.models::tr_models_roc(dados = dados, resposta = resposta, probabilidade = sub("^\\.", "", probabilidade),
                              permitir_treino = permitir_treino)
}
tr_ml_pr_curve <- function(dados, resposta, probabilidade, permitir_treino = FALSE) {
  trama.models::tr_models_pr_curve(dados = dados, resposta = resposta, probabilidade = sub("^\\.", "", probabilidade),
                                   permitir_treino = permitir_treino)
}
...
test_that("avaliar o treino: erro por padrão, nota e aviso com permitir_treino", {
  ...
  expect_error(tr_ml_evaluate(pt, "Species", permitir_treino = NA), class = "tr_models_error_bad_option")
  ...
  expect_equal(cf[setdiff(names(cf), "nota")], tr_ml_confusion(sem, "Species"), ignore_attr = TRUE)
...
test_that("marca sobrevive ao store e à releitura do cache no fluxo real", {
  ...
  f <- trama::tr_flow(reg) |>
    ...
    trama::tr_add("errado", "ml/cart", resposta = "Species", from = "divisao:teste") |>
    trama::tr_add("modelo", "ml/cart", resposta = "Species", from = "divisao:treino") |>
    trama::tr_add("no_treino", "models/predict", from = c("modelo", "divisao:treino")) |>
    trama::tr_add("aval_treino", "models/evaluate", resposta = "Species", from = "no_treino") |>
    trama::tr_add("aval_treino_ok", "models/evaluate", resposta = "Species", permitir_treino = TRUE,
                  from = "no_treino") |>
    trama::tr_add("no_teste", "models/predict", from = c("modelo", "divisao:teste")) |>
    trama::tr_add("aval_teste", "models/evaluate", resposta = "Species", from = "no_teste")
