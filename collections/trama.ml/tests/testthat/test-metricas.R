...
test_that("kappa de Cohen: casos de borda", {
  ...
  expect_equal(val(tr_ml_evaluate(data.frame(y = c("a", "b"), .pred = c("a", "b")), "y"), "kappa"), 1)
  expect_equal(val(tr_ml_evaluate(data.frame(y = c("a", "b", "b"), .pred = c("b", "b", "b")), "y"), "kappa"), 0)
  expect_true(is.na(val(tr_ml_evaluate(data.frame(y = c("a", "a"), .pred = c("a", "a")), "y"), "kappa")))
})
...
test_that("yardstick confere macro, ponderadas e kappa", {
  ...
  z <- tr_ml_evaluate(data.frame(y = y, .pred = p), "y")
  ...
  zb <- tr_ml_evaluate(data.frame(y = as.character(b$truth), .pred = as.character(b$est)), "y")
...
test_that("precisão de classe nunca prevista é NA e sai das médias", {
  ...
  z <- tr_ml_evaluate(data.frame(y = c("a", "a", "b", "b", "c"),
                                 .pred = c("a", "a", "b", "b", "b")), "y")
  ...
  w <- tr_ml_evaluate(data.frame(y = c("a", "b"), .pred = c("z", "z")), "y")
