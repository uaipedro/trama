...
nos_multi <- function(reg) Filter(function(n) startsWith(n$id, "multi/"), reg$nodes)

test_that("a coleção carrega sobre data e view, e não sozinha", {
  expect_no_error(multi_registry())
...
test_that("o catálogo sai com a ajuda e os adaptadores", {
  cat_json <- as.character(trama::tr_catalog_json(multi_registry()))
  expect_match(cat_json, "Exemplo multivariado", fixed = TRUE)
  expect_match(cat_json, "\"from\":\"multi/pca\"", fixed = TRUE)
})
...
test_that("o fn de todo nó está exportado no NAMESPACE", {
  ...
  for (n in nos_multi(multi_registry())) {
