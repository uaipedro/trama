...
test_that("kappa bate com irr e psych nas previsões do iris (3 grupos)", {
  ...
  r <- trama.models::tr_models_confusion(l, tabela = "métricas")
...
test_that("a tabela padrão continua a matriz, e a opção é conferida", {
  l <- tr_multi_discriminant(iris, resposta = "Species")
  expect_equal(names(trama.models::tr_models_confusion(l))[1], "real")
  r <- trama.models::tr_models_confusion(l, tabela = "métricas")
  expect_equal(names(r), c("medida", "grupo", "valor"))
  expect_error(trama.models::tr_models_confusion(l, tabela = "tudo"), class = "tr_models_error_bad_option")
})
