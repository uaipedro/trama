...
test_that("coeficientes, EP, intervalos perfilados e p batem com o logistf no sex2", {
  d <- sex2_trama()
  o <- oraculo_logistf(d)
  m <- tr_multi_logistic(d, resposta = "case", preditores = "age, oc, vic, vicl, vis, dia", metodo = "firth")
...
test_that("Firth prevê, classifica, valida por deixa-um-fora e passa no jackknife", {
  ...
  fi <- tr_multi_logistic(p, resposta = "diabetes", metodo = "firth")
  ml <- tr_multi_logistic(p, resposta = "diabetes")
  ...
  cv <- trama.models::tr_models_confusion(fi, validacao = "cruzada")
  expect_s3_class(cv, "tbl_df")
  expect_error(tr_multi_logistic(tr_multi_example("iris"), resposta = "Species", metodo = "firth"),
               class = "tr_multi_error_bad_option")
  expect_error(tr_multi_logistic(p, resposta = "diabetes", metodo = "bayes"),
               class = "tr_multi_error_bad_option")
})
...
test_that("perfil difícil (vinhos B × C, quase separado): limites são cruzamentos do χ²₁", {
  ...
  v <- v[v$cultivar != "A", ]
  f <- tr_multi_logistic(v, resposta = "cultivar", preditores = "alcool, flavonoides, intensidade_cor",
                         metodo = "firth")
...
test_that("ajuda da models/coefficients (antes logistic_coefficients) avisa que z é Wald e p é perfilado", {
  h <- multi_registry()$nodes[["models/coefficients"]]$help
