...
test_that("oráculos: pima por deixa-um-fora (binária) e vinhos (cada grupo contra os outros)", {
  ...
  v <- tr_multi_discriminant(tr_multi_example("vinhos"), grupo = "cultivar")
...
test_that("o bloco: subtítulo com AP e acaso (binária), legenda por grupo (3+), bordas", {
  ...
  expect_error(tr_multi_pr_curve(m, validacao = "x"), class = "tr_multi_error_bad_option")
  expect_error(tr_multi_pr_curve(iris), class = "tr_multi_error_not_a_classifier")
})

test_that("o nó está registrado com o tipo multi/classifier", {
  reg <- multi_registry()
  spec <- reg$nodes[["multi/pr_curve"]]
  expect_false(is.null(spec))
  expect_identical(spec$inputs$modelo$type, "multi/classifier")
})
