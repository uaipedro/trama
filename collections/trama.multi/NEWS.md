...
Versão sobe de 0.2.0 para 0.3.0: a integração com a coesão das coleções
muda o que os blocos devolvem (a discriminante e a logística saem como
`models/fit`, e classificar, confundir, ROC, PR e coeficientes passam aos
blocos da `trama.models`).

## Integração 9.1b (coesão × main)

- `multi/logistic` ganha `metodo = "firth"` (main); os coeficientes (com IC
  perfilado e p da razão de verossimilhanças, ou Wald) saem pelo
  `models/coefficients`, e fluxos com `multi/logistic_coefficients` (inclusive
  com `nivel`, v2, ou `intervalo`, v4) abrem migrados para ele.
- `multi/roc` e `multi/pr_curve` migram para `models/roc` e `models/pr_curve`,
  com DeLong, Hand & Till e as curvas por grupo portadas; `multi/confusion`
  migra para `models/confusion` com `tabela`.
- Jackknife por grupo (`grupo`) e `confianca` (v2) ficam aqui; as duas
  migrações (da coleção e por versão do nó) convivem.
