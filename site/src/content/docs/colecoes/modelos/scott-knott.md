...
Para dividir muitos tratamentos em grupos que não se sobrepõem, sobre uma ANOVA balanceada (DIC, DBC, DQL, fatorial, parcela subdividida). O alfa vale para cada corte, não para o agrupamento inteiro. Com bloco incompleto, covariável ou repetições desiguais o bloco recusa: as médias da tabela deixam de ter a variância comum que o teste supõe. Nesses casos, use o `models/emmeans`.

## Configuração

Informe o tratamento e o nível (padrão 5%). Na parcela subdividida, o bloco usa o erro (a) ou (b) do fator.
